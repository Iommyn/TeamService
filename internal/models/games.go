...
var GameWhere = struct {
	...
	ID:          whereHelperint64{field: "\"games\".\"id\""},
	Name:        whereHelperstring{field: "\"games\".\"name\""},
	Description: whereHelperstring{field: "\"games\".\"description\""},
	UpdateAt:    whereHelpertime_Time{field: "\"games\".\"update_at\""},
...
type gameR struct {
	GameMaps    GameMapSlice    `boil:"GameMaps" json:"GameMaps" toml:"GameMaps" yaml:"GameMaps"`
	Tournaments TournamentSlice `boil:"Tournaments" json:"Tournaments" toml:"Tournaments" yaml:"Tournaments"`
}
...
func (r *gameR) GetGameMaps() GameMapSlice {
	if r == nil {
		return nil
	}
	return r.GameMaps
...
func (r *gameR) GetTournaments() TournamentSlice {
	if r == nil {
		return nil
	}
...
var (
	gameAllColumns            = []string{"id", "name", "description", "update_at", "created_at"}
	gameColumnsWithoutDefault = []string{"name", "description"}
	gameColumnsWithDefault    = []string{"id", "update_at", "created_at"}
...
// GameMaps retrieves all the game_map's GameMaps with an executor.
func (o *Game) GameMaps(mods ...qm.QueryMod) gameMapQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"game_map\".\"game_id\"=?", o.ID),
	)

	return GameMaps(queryMods...)
}

// Tournaments retrieves all the tournament's Tournaments with an executor.
func (o *Game) Tournaments(mods ...qm.QueryMod) tournamentQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"tournaments\".\"game_id\"=?", o.ID),
	)

	return Tournaments(queryMods...)
}

// LoadGameMaps allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (gameL) LoadGameMaps(ctx context.Context, e boil.ContextExecutor, singular bool, maybeGame interface{}, mods queries.Applicator) error {
	var slice []*Game
	var object *Game
	...
	query := NewQuery(
		qm.From(`game_map`),
		qm.WhereIn(`game_map.game_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load game_map")
	}

	var resultSlice []*GameMap
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice game_map")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on game_map")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.GameID {
				local.R.GameMaps = append(local.R.GameMaps, foreign)
				if foreign.R == nil {
					foreign.R = &gameMapR{}
				}
...
// LoadTournaments allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (gameL) LoadTournaments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeGame interface{}, mods queries.Applicator) error {
	var slice []*Game
	var object *Game
	...
	query := NewQuery(
		qm.From(`tournaments`),
		qm.WhereIn(`tournaments.game_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.Tournaments = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.GameID {
				local.R.Tournaments = append(local.R.Tournaments, foreign)
				if foreign.R == nil {
					foreign.R = &tournamentR{}
				}
				foreign.R.Game = local
...
// AddGameMaps adds the given related objects to the existing relationships
// of the game, optionally inserting them as new records.
// Appends related to o.R.GameMaps.
// Sets related.R.Game appropriately.
func (o *Game) AddGameMaps(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*GameMap) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.GameMaps = append(o.R.GameMaps, related...)
	}
...
// AddTournaments adds the given related objects to the existing relationships
// of the game, optionally inserting them as new records.
// Appends related to o.R.Tournaments.
// Sets related.R.Game appropriately.
func (o *Game) AddTournaments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Tournament) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.GameID = o.ID
	...
	if o.R == nil {
		...
		o.R.Tournaments = append(o.R.Tournaments, related...)
	}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(gamePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert games, could not build conflict column list")
			}
