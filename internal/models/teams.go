...
type Team struct {
	...
	TeamLogo    string    `boil:"team_logo" json:"team_logo" toml:"team_logo" yaml:"team_logo"`
...
var TeamWhere = struct {
	...
	TeamLogo    whereHelperstring
	Description whereHelperstring
	UpdatedAt   whereHelpertime_Time
	CreatedAt   whereHelpertime_Time
}{
	ID:          whereHelperint64{field: "\"teams\".\"id\""},
	UserID:      whereHelperint64{field: "\"teams\".\"user_id\""},
	TeamName:    whereHelperstring{field: "\"teams\".\"team_name\""},
	TeamLogo:    whereHelperstring{field: "\"teams\".\"team_logo\""},
	Description: whereHelperstring{field: "\"teams\".\"description\""},
...
type teamR struct {
	TeamsStats TeamsStatSlice `boil:"TeamsStats" json:"TeamsStats" toml:"TeamsStats" yaml:"TeamsStats"`
	TeamsUsers TeamsUserSlice `boil:"TeamsUsers" json:"TeamsUsers" toml:"TeamsUsers" yaml:"TeamsUsers"`
}
...
func (r *teamR) GetTeamsStats() TeamsStatSlice {
	if r == nil {
		return nil
	}
	return r.TeamsStats
...
func (r *teamR) GetTeamsUsers() TeamsUserSlice {
	if r == nil {
		return nil
	}
	return r.TeamsUsers
...
var (
	teamAllColumns            = []string{"id", "user_id", "team_name", "team_logo", "description", "updated_at", "created_at"}
	teamColumnsWithoutDefault = []string{"user_id", "team_name", "team_logo", "description"}
	teamColumnsWithDefault    = []string{"id", "updated_at", "created_at"}
...
// TeamsStats retrieves all the teams_stat's TeamsStats with an executor.
func (o *Team) TeamsStats(mods ...qm.QueryMod) teamsStatQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"teams_stats\".\"team_id\"=?", o.ID),
	)

	return TeamsStats(queryMods...)
}

// TeamsUsers retrieves all the teams_user's TeamsUsers with an executor.
func (o *Team) TeamsUsers(mods ...qm.QueryMod) teamsUserQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"teams_users\".\"team_id\"=?", o.ID),
	)

	return TeamsUsers(queryMods...)
}

// LoadTeamsStats allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (teamL) LoadTeamsStats(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTeam interface{}, mods queries.Applicator) error {
	var slice []*Team
	var object *Team
	...
	query := NewQuery(
		qm.From(`teams_stats`),
		qm.WhereIn(`teams_stats.team_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load teams_stats")
	}

	var resultSlice []*TeamsStat
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice teams_stats")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on teams_stats")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for teams_stats")
	}
	...
	if singular {
		object.R.TeamsStats = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &teamsStatR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.TeamID {
				local.R.TeamsStats = append(local.R.TeamsStats, foreign)
				if foreign.R == nil {
					foreign.R = &teamsStatR{}
				}
...
// LoadTeamsUsers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (teamL) LoadTeamsUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTeam interface{}, mods queries.Applicator) error {
	var slice []*Team
	var object *Team
	...
	query := NewQuery(
		qm.From(`teams_users`),
		qm.WhereIn(`teams_users.team_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load teams_users")
	}

	var resultSlice []*TeamsUser
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice teams_users")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on teams_users")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for teams_users")
	}
	...
	if singular {
		object.R.TeamsUsers = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &teamsUserR{}
			}
			foreign.R.Team = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.TeamID {
				local.R.TeamsUsers = append(local.R.TeamsUsers, foreign)
				if foreign.R == nil {
					foreign.R = &teamsUserR{}
				}
				foreign.R.Team = local
...
// AddTeamsStats adds the given related objects to the existing relationships
// of the team, optionally inserting them as new records.
// Appends related to o.R.TeamsStats.
// Sets related.R.Team appropriately.
func (o *Team) AddTeamsStats(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TeamsStat) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.TeamsStats = append(o.R.TeamsStats, related...)
	}
...
// AddTeamsUsers adds the given related objects to the existing relationships
// of the team, optionally inserting them as new records.
// Appends related to o.R.TeamsUsers.
// Sets related.R.Team appropriately.
func (o *Team) AddTeamsUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TeamsUser) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.TeamID = o.ID
	...
	if o.R == nil {
		...
		o.R.TeamsUsers = append(o.R.TeamsUsers, related...)
	}
